...
export function ProjectFilesManager({
  ...
  return (
    <div className="w-full flex flex-col h-full bg-[#fafafa] overflow-hidden font-sans">
      {/* Header Bar */}
      <div className="h-13 px-4 bg-white border-b border-[#e4e4e7] flex items-center justify-between z-20 shrink-0 select-none">
        <div className="flex items-center gap-2">
          <Folder className="w-4 h-4 text-[#BA4E20]" />
          <h2 className="text-xs font-semibold text-[#09090b]">Arquivos do projeto</h2>
        </div>

        <button
          onClick={onClose}
          className="p-1 hover:bg-[#f4f4f5] rounded-lg text-[#71717a] hover:text-[#09090b] transition-colors cursor-pointer"
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 w-full">
        ...
        <div
          ...
          className={`cursor-pointer p-5 border-2 border-dashed rounded-xl text-center transition-all flex flex-col items-center justify-center gap-2 bg-white shadow-2xs ${
            isDragOver
              ? 'border-[#BA4E20] bg-[#fdf5f2]'
              : 'border-[#e4e4e7] hover:border-[#BA4E20]/60 hover:bg-[#fdf5f2]/30'
          }`}
        >
          <div className="p-2.5 bg-[#fdf5f2] border border-[#BA4E20]/20 rounded-lg text-[#BA4E20]">
            <UploadCloud className="w-5 h-5" />
          </div>
          <div>
            <h3 className="text-xs font-medium text-[#09090b]">
        ...
        {files.length > 0 && (
          <div className="relative w-full">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 bg-white border border-[#e4e4e7] rounded-lg text-xs text-[#09090b] placeholder-[#a1a1aa] focus:outline-none focus:border-[#BA4E20] shadow-2xs"
            />
          </div>
        )}
          ...
          <div className="space-y-2">
            {filteredFiles.map((file) => {
              ...
              return (
                <div
                  key={file.id}
                  className="group bg-white border border-[#e4e4e7] hover:border-[#BA4E20]/40 rounded-xl p-3 transition-all shadow-2xs flex items-center justify-between gap-3 relative"
                >
                  <div className="flex items-center gap-2.5 min-w-0 flex-1">
                    <div className="p-2 bg-[#f4f4f5] border border-[#e4e4e7] rounded-lg shrink-0">
                      {getFileIcon(file)}
                    </div>
                    <div className="min-w-0 flex-1">
                      <h4 className="text-xs font-medium text-[#09090b] truncate" title={file.name}>
                        {file.name}
                      </h4>
                  ...
                  <div className="relative shrink-0">
                    <button
                      ...
                      className={`p-1.5 rounded-lg text-[#71717a] hover:text-[#09090b] hover:bg-[#f4f4f5] transition-colors cursor-pointer ${
                        isMenuOpen ? 'bg-[#f4f4f5] text-[#09090b]' : 'hidden group-hover:block'
                      }`}
